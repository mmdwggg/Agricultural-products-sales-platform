  ...
  <div class="navigation-bar">
    ...
    <div class="menu-content" v-cloak>
      <el-menu
        ...
        mode="horizontal"
        @select="handleSelect"
        background-color=" rgb(125,170,61)"
        text-color="#fff"
        ...
        <el-submenu index="8" v-else>
          ...
          <el-menu-item index="8-5" @click.native="tjChart" v-if="$store.getters.isAdmin">统计报表</el-menu-item>
          <el-menu-item index="8-8" @click.native="handleWeather">天气预报</el-menu-item>
          <el-menu-item index="8-9" @click.native="handleChat">我的客服</el-menu-item>
          <el-menu-item index="8-6" @click.native="handleAbout">关于我们</el-menu-item>
...
export default {
  ...
  methods: {
    ...
    handleWeather () {
      this.$router.push("/home/weather").catch((err) => err);
    },
    handleChat () {
      this.$router.push("/home/machineChat").catch((err) => err);
    },
    goodsManage(){
      this.$router.push("/home/userGood/publishedgoodsAdmin").catch((err) => err)
    },
    tjChart(){
       this.$router.push("/home/chartJs").catch((err) => err);
    },
    userManage(){
...
.navigation-bar {
  ...
  .menu-content{
    height: 42px;
    background-color:  rgb(125,170,61) !important;
  }
  .el-menu {
    width: 1100px;
    margin: 0 auto;
